...
import os
from app.backend_tools import init_storage_path, add_timestamp, read_config
from app.worker_ant import worker_ant
app = FastAPI()

#чтение yaml конфига
config = read_config()

#создание потока с воркерами
mp_ctx = mp.get_context('spawn')
audio_file_tasks_quque = mp_ctx.Queue()
errors_from_process = mp_ctx.Queue()
p = [mp_ctx.Process(target=worker_ant, args=(audio_file_tasks_quque,errors_from_process,)) for _ in range(config['WORKER_NUM'])]
for worker in p:
    worker.start()
...
def startup_model():
    ...
    for _ in range(config['WORKER_NUM']): #ждём готовности всех воркеров
        is_ready = errors_from_process.get()
        assert(is_ready)
    model_ready = True
